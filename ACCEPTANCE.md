...
| ⚠️ | Model weights pinned, **but not yet checked against the publisher** | The digests are pinned and Desk boots against them (`40028e43…` config.json, `1bb29b03…` weights.npz, revision `52a88bf6…`). What is NOT done is confirming those are the digests the publisher published — right now they attest only that the file has not changed **since Joel downloaded it**, which does not rule out a bad download. This cannot be checked from the build container: `huggingface.co` is outside its egress policy. On the Mac: `curl -sI https://huggingface.co/mlx-community/whisper-small.en-mlx/resolve/52a88bf6e98b114a210c21bb83e22d6e1505cb73/weights.npz \| grep -i x-linked-etag` returns the published SHA-256 for the LFS blob; it must equal the pinned `weights.npz` digest. `config.json` is not an LFS object, so its ETag is a git blob SHA-1 rather than a SHA-256 — compare it by re-fetching the file and hashing it. |
